...
### Slice 6 RDD correction 1 — envelope-hash cache fail-open (commit `1cafa01`)

Native RDD review (`gentle-ai review`, lineage `review-928ca98a47d13cf5`, granted by the user, high risk, 76 files/7509 lines against `main`) ran its four lenses on the post-refactor tree. `review-readability` returned `correction_required` with three corroborated findings: **R1**/**R3** (BLOCKER, risk+reliability — the fiscal runtime mode gate, see the next section) and **R4** (CRITICAL, resilience).

- **R4 — `refreshEnvelopeCache` populated `FiscalLinks` via the tolerant `fiscalBindingLinkContributionsBestEffort` helper**, which swallows any `fiscal_binding_links` query error and degrades to "no links"; the resulting (possibly wrong) envelope hash was then persisted unconditionally. A transient read failure on a v1 fiscal-bound subject would silently persist an incorrect, fiscal-links-omitted cached hash instead of aborting — masking a transient store error as a successful write for exactly the class of record this feature exists to make tamper-evident. `ImportObservation` reads this persisted value back for sync duplicate/conflict classification, so the defect was not cosmetic.
- **Fix:** `refreshEnvelopeCache` now uses the error-propagating `fiscalBindingLinkContributionsTx` and aborts (returns the error) instead of persisting on a genuine read failure. Every existing caller (`Save`, `AddEvidenceLinksBound`, `AddRuleLinksBound`, the structured-rule-link path) already checked this return value inside a transaction that rolls back on error, so no caller needed a change.
- **Regression caught before commit:** a first version of this fix failed closed on ANY read error, which broke `TestMigrationV18IsAdditiveAndFailsClosed` — a test that deliberately exercises a pre-v18 schema where `fiscal_binding_links` does not exist at all (a permanent, non-transient condition that must keep degrading to "no links"). Added `fiscalLinksTableExists`, a single `sqlite_master` catalog lookup, to distinguish "table absent (pre-v18, tolerate)" from "table present but the read failed (v18+, abort)" — SQLite reports both as the generic `SQLITE_ERROR(1)` result code, so a catalog check is used instead of sniffing the error.
- Three new tests in `internal/store/envelope_cache_fiscal_test.go` cover: an existing-but-corrupted table aborts the refresh and leaves `envelope_hash` unchanged; a genuinely link-free subject still refreshes; a missing (pre-v18) table still tolerates exactly as before.
- **R1/R3 were deliberately NOT fixed in this correction** — see the next section for why and how they were later closed separately.
- **Verification:** `go build`, `go vet`, `gofmt -l` clean; `go test` green for `internal/core`, `internal/authz`, `internal/server`, `internal/store`, `cmd/drenyra-engram` (including the previously-regressed migration test).

### Slice 6 RDD correction 2 — fiscal runtime mode gate wired + observability (commit `9be1a66`)

**R1/R3 (BLOCKER, risk+reliability, same review as above):** `FiscalRuntimeMode`/`ParseFiscalRuntimeMode`/`(FiscalRuntimeMode).CheckProtectedWrite` (`internal/store/fiscal_scope_store.go`) were fully defined but never called from any production write path — confirmed by `grep`: the only call sites were the isolated unit test `TestFiscalRuntimeModesAndDowngradeRefusal`. Design.md's documented default-safe rollout ("shadow mode fails every protected v1 or legacy write closed until an operator explicitly selects `enforce`/`legacy_compat`") therefore had no effect: any caller with a structurally valid, axis-matching `FiscalWriteIntent` could create/mutate v1 fiscal-bound state immediately, regardless of the configured runtime mode.

Not fixed in correction 1 because an empirical, reverted experiment proved the true blast radius: gating even one entry point (`Save`) under the (universal, since no test sets the env var) default `shadow` crashed the entire `internal/store` test suite with a panic in ~4 seconds. Closing this properly required a coordinated test-infrastructure migration, which was out of scope for a single bounded review correction and was instead done as deliberate follow-up work (delegated to a subagent with a detailed brief; independently re-verified rather than trusted on its self-report):

- `SQLiteStore` gained a `fiscalRuntimeMode` field, resolved once in `openInternal` from `DRENYRA_FISCAL_RUNTIME_MODE` (empty → shadow; an unrecognized value fails `Open`, per design.md "Unknown values fail startup").
- `checkFiscalRuntimeGate` — the single shared choke point — is now called from all 5 real protected-write entry points (`Save`, `ApproveMemory`, `SupersedeExplicit`, `addLinksBound`, `storeObject`/`StoreObjectWithFiscalIntent`), scoped to `ScopeKindCompany` only (institutional memories exempt per proposal.md's "protected fiscal operations" definition).
- **A second defect found and fixed before commit:** `checkFiscalRuntimeGate` initially ran before `beginWriteTx`'s drill write-freeze latch check. A corruption-drill evidence copy is built via a raw struct literal that bypasses `openInternal`, so its `fiscalRuntimeMode` is permanently the zero value (shadow-equivalent) — every write on a frozen drill copy therefore reported the misleading `FISCAL_WRITE_GATE_CLOSED` instead of the specific `ErrStoreWriteFrozen` a caller may depend on, masking exactly the behavior `TestRunCorruptionDrillFullPath` verifies. Fixed by checking `s.writeFrozen.Load()` first, inside `checkFiscalRuntimeGate` itself, rather than duplicating the check at all 5 call sites.
- 22 test files migrated to an explicit `DRENYRA_FISCAL_RUNTIME_MODE` (`legacy_compat` for plain legacy fixtures via `newTestStore`'s new default, `enforce` for fiscal-intent-carrying tests via `newTestStoreMode`, close+reopen under each mode in turn — via `reopenTestStoreMode` — when one dataset genuinely needs both classes, since no single mode permits both). Zero test assertions changed; only store-bootstrap/env setup.
- `internal/store/fiscal_runtime_gate_wiring_test.go` (new) proves the gate through the public `Save` entry point — shadow denies with `FISCAL_WRITE_GATE_CLOSED`, `enforce` allows a v1 (FiscalIntent-carrying) save while denying a legacy one with `SCOPE_BINDING_REQUIRED` — rather than only the isolated `CheckProtectedWrite` unit test.

**A second, separate RDD review** (lineage `review-70931ebc5d9a7d85`, granted by the user, 27 files/663 lines) ran against the uncommitted wiring above. `review-reliability` returned `correction_required` with four findings (R3-1 CRITICAL, R4-1 BLOCKER, R4-2/R4-3 CRITICAL) all describing the same underlying fact: the newly-activated `shadow` default now fails every company-scoped protected write with no startup warning, and an invalid `DRENYRA_FISCAL_RUNTIME_MODE` value now fails `Open()` entirely.

**These findings were verified against design.md and found to describe INTENTIONAL, explicitly-mandated behavior, not defects** — "every protected v1 or legacy write fails `FISCAL_WRITE_GATE_CLOSED`" in shadow, and "Unknown values fail startup," are both direct quotes. Weakening either would have regressed the R1/R3 fix this correction exists to make. What the findings correctly identified as genuinely missing was **observability**, not permissiveness:

- Added `SQLiteStore.FiscalRuntimeMode()`, a read-only getter granting no authority.
- `openStoreWithRoot` in `cmd/drenyra-engram/main.go` — the single choke point every CLI command and the HTTP/MCP `serve`/`mcp` commands open the store through — now prints one stderr diagnostic when the resolved mode is `shadow`, naming the variable and the code a blocked write will see. stderr only, never stdout, so it cannot corrupt a command's machine-readable JSON output.
- Two new tests (`cmd/drenyra-engram/fiscal_runtime_mode_diagnostic_test.go`) prove the diagnostic fires only for the unconfigured default and never for an explicitly configured mode.

The corrected candidate (target `sha256:74366e67...`, lineage `review-8ef2d7e4da1a3a00`, the full branch — 90 files/8358 lines — against `main` after committing the above) was re-offered for review and **declined by the user** (`consent: "declined_this_candidate"`) — no review record was created for that final candidate. The two prior reviews' findings (R4, R1/R3) were genuinely fixed and verified regardless; the decline only means no receipt exists for the combined final state.

- **Verification (commit `9be1a66`):** `go build`, `go vet`, `gofmt -l` clean; `go test` green for `internal/core`, `internal/authz`, `internal/server`, `internal/store`, `cmd/drenyra-engram`.
