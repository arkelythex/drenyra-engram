...
| G. Migrations | 111–122 | PASS — direct-upgrade matrix v1…v13→v14 through the normal `Open` path (`TestDirectUpgradeMatrixV1ToV14`, `internal/store/migration_direct_upgrade_test.go`), deterministic interruption rollback + safe reopen convergence (`TestMigrationInterruptedRollsBackToPriorVersion`, `TestMigrationCrashReopenConvergesToV14`), frozen doc-only provenance (`contracts/provenance.md` § Migration provenance for schema v14 (frozen)); no migration-history table or schema v15 (FD-2) | Closed — matrix, crash-recovery proof, and provenance decision delivered with executable citations |
...
| J. Authorization | 159–175 | PASS — consolidated versioned policy matrix freezing `approval-policy/v0.4.0`, `judgment-policy/v0.4.0`, `reconciliation-policy/v0.5.0`, `evidence-lifecycle-policy/v0.8.0` with dual-approval and SoD denials (`TestConsolidatedVersionedPolicyMatrix`, `internal/authz/consolidated_policy_matrix_test.go`); no override field/flag/input on HTTP, MCP, or CLI surfaces (`TestNoOverrideFieldOnRequestSurfaces`, `TestCLINoOverrideFlagOnCommands`); break-glass inputs fail closed with zero state change and denials are unbypassable at policy/adapter/store boundaries (`TestOverrideInputDeniedFailClosed`, `TestDeniedOperationNotBypassableViaAdapterOrStore`); override absence frozen as negative conformance (`contracts/provenance.md` § Frozen override-absence decision). PASS bounded to the engine's professional approval policy — no business/payment authorization claim and no production identity readiness claim (block I retained unchanged) | Closed — versioned matrix, negative override conformance, and SoD evidence delivered and cited |
...
| L. Idempotency | 186–194 | PASS — consolidated operation × surface replay matrix with named rows, replay semantics, changed-payload/principal conflicts, and no-duplicate-effect snapshots (`TestIdempotencyReplayMatrix`, `internal/server/idempotency_replay_matrix_test.go`); MCP replay (`TestMCPPurgeRequestIdempotentReplay`, `TestMCPJudgmentProposeIdempotentReplay`, `TestMCPReconciliationProposeIdempotentReplay`); CLI replay (`TestCLIJudgeReplay`, `TestCLIReconcileReplay`, `TestCLIReviewRejectReplay`, `TestCLIPurgeRequestReplay`); lost-response/interrupted-reservation recovery for approval, judgment, reconciliation, review, hold, and reopen (`internal/store/idempotency_interrupted_reservation_test.go`); sync no-op second import (`TestSyncIdempotent`, `TestCLISyncRoundTrip`) | Closed — replay matrix, conflict rows, no-duplicate-effect proof, and lost-response evidence delivered and cited |
...
| Q. Multi-tenant security | 277–296 | PASS — generated cross-tenant matrix over the canonical API surface with an exhaustiveness guard (`TestCrossTenantMatrix`, `TestCrossTenantMutationNoSideEffect`, `TestCrossTenantMatrixExhaustiveness`, `internal/server/cross_tenant_matrix_test.go`): every operation proven nonexistence-safe from tenant B with zero foreign-identifier leakage and zero side effect on denied mutations; exact-scope enforcement at HTTP, MCP, and store boundaries (`internal/server/http.go`, `internal/server/mcp.go`, `internal/store/store.go`) — the adapter-level check is CALLER-ASSERTED scope equality (the caller supplies its exact scope tuple; binding a verified identity to that scope is a production identity prerequisite tracked in block I / issue #18, NOT claimed here), while store-level scope filtering is structural; search leakage probes hard-fail at EXACTLY 0 (`internal/search/bench/queries.go`, `internal/search/bench/bench_test.go`) | Closed — generated matrix + exhaustiveness guard delivered; no checked-in JSON registry (FD-4) |
...
| Z. Operability | 420–433 | PASS for repository-demonstrated operability / RISK for deployment objectives — doctor, WAL-safe snapshot/restore, marked-copy corruption drills, scope + backup-identity verification, and tenant export are delivered and tested (`docs/architecture/operability-evidence.md`; `internal/store/doctor.go`, `internal/store/drill.go`, `internal/store/evidence_export_test.go`); numerical RTO/RPO targets remain deployment/business-owned and UNKNOWN (FD-3) | Closed — operability evidence document with executable citations; recovery objectives stay qualitative and owner-owned |
