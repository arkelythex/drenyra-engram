...
| G-5 | Evidence objects have hash, retention, availability, legal-hold, export semantics | — | PARTIAL (hash + availability DELIVERED v0.7.0; retention policies + legal holds + approved purge pipeline + lifecycle export DELIVERED v0.8.0 — schema v12; repository-local snapshot/restore + corruption drills DELIVERED — [operability-evidence.md](../architecture/operability-evidence.md); **still DEFERRED: cloud/remote object storage, scheduler executor, OCR/content search**) | [docs/architecture/evidence-lifecycle-v0.8.md](../architecture/evidence-lifecycle-v0.8.md); [docs/security/evidence-lifecycle-and-threat-model.md](../security/evidence-lifecycle-and-threat-model.md) |
| G-6 | Cross-tenant, race, fuzz, restore, and corruption drills pass | — | PARTIAL (cross-tenant structural + race coverage exist; restore + corruption drills are repository-delivered — [operability-evidence.md](../architecture/operability-evidence.md), `internal/store/drill_test.go` `TestRunRestoreDrillSuccess`/`TestRunCorruptionDrillFullPath`; fuzz coverage and full race/cross-tenant closure remain open — bounded PARTIAL retained) | [contracts/scope.md](../../contracts/scope.md); audit Blocks G, K, Q, Z |
