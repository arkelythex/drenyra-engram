...
func TestFreshStoreBootstrapsV5ReceiptTables(t *testing.T) {
	...
	if version != 14 {
		t.Fatalf("schema_version = %d, want 14 (the chain continues v5→v6→v7→v8→v9→v10→v11→v12→v13→v14)", version)
	}
...
func TestV4StoreMigratesToV5AdditivelyPreservingRows(t *testing.T) {
	...
	if version != 14 {
		t.Fatalf("schema_version after migration = %d, want 14 (the chain continues v5→v6→v7→v8→v9→v10→v11→v12→v13→v14)", version)
	}
