...
func TestFreshStoreBootstrapsV6ClosureTables(t *testing.T) {
	...
	if version != 8 {
		t.Fatalf("schema_version = %d, want 8 (the chain continues v6→v7→v8)", version)
	}
...
func TestV5StoreMigratesToV6AdditivelyPreservingRows(t *testing.T) {
	...
	if version != 8 {
		t.Fatalf("schema_version after migration = %d, want 8 (the chain continues v6→v7→v8)", version)
	}
