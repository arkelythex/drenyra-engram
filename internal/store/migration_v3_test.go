...
func TestFreshStoreBootstrapsToSchemaV5(t *testing.T) {
	...
	if version != 16 {
		t.Fatalf("schema_version = %d, want 16 (the chain continues v3→v4→v5→v6→v7→v8→v9→v10→v11→v12→v13→v14)", version)
	}
...
func TestV2StoreMigratesToV5AdditivelyPreservingRows(t *testing.T) {
	...
	if version != 16 {
		t.Fatalf("schema_version after migration = %d, want 16 (the chain continues v3→v4→v5→v6→v7→v8→v9→v10→v11→v12→v13→v14)", version)
	}
