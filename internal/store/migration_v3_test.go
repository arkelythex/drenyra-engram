...
func TestFreshStoreBootstrapsToSchemaV5(t *testing.T) {
	...
	if version != 6 {
		t.Fatalf("schema_version = %d, want 6 (the chain continues v3→v4→v5→v6)", version)
	}
...
func TestV2StoreMigratesToV5AdditivelyPreservingRows(t *testing.T) {
	...
	if version != 6 {
		t.Fatalf("schema_version after migration = %d, want 6 (the chain continues v3→v4→v5→v6)", version)
	}
