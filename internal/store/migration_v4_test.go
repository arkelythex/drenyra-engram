...
func TestFreshStoreBootstrapsV4JudgmentPersistence(t *testing.T) {
	...
	if version != 10 {
		t.Fatalf("schema_version = %d, want 10 (the chain continues v4→v5→v6→v7→v8→v9→v10)", version)
	}
...
func TestV3StoreMigratesToV5AdditivelyPreservingRows(t *testing.T) {
	...
	if version != 10 {
		t.Fatalf("schema_version after migration = %d, want 10 (the chain continues v4→v5→v6→v7→v8→v9→v10)", version)
	}
