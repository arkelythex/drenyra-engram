...
func TestMCPToolsList(t *testing.T) {
	...
	if len(result.Tools) != 58 {
		// 13 engram_* + 45 accounting_* (current context in v0.5, object storage
		...
		// the v0.9.0 review workspace: queue/detail/reject/return, the
		// v0.6.0 fiscal-policy rule surfaces: show/history/impact, and the
		// v1-readiness G-10 accounting_reconstructibility read-only metric).
		t.Fatalf("tool count = %d, want 58 (13 engram_* + 45 accounting_*)", len(result.Tools))
	}
