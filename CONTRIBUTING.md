...
**Status: pre-alpha.** Drenyra Engram is extracted from `arkelythex/Drenyra` (`packages/memory`, `packages/agent-memory`) through vertical slices. The maintainer (Arkelythex) drives the extraction; external contributions are welcome only after the contracts in `contracts/` stabilize.
