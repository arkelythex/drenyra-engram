...
const TWENTY_FOUR_ACTIONS: readonly ReceiptAction[] = [
	"memory_recorded",
	"memory_approved",
	"memory_rejected",
...
describe("receipt protocol mirror (v0.4.0 Step 3)", () => {
	it("exposes the closed subject and action sets", () => {
		expect(RECEIPT_SUBJECT_TYPES).toEqual([
			"memory",
			"judgment",
			"reconciliation",
			"evidence_object",
		]);
		expect(RECEIPT_ACTIONS).toEqual([...TWENTY_FOUR_ACTIONS]);
		expect(RECEIPT_ACTIONS.length).toBe(24);
